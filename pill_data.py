...
class Data_pil:
    def __init__(self):
        super(Data_pil, self).__init__()
        self.create_pill()

    def create_pill(self):
        db = sqlite3.connect("server.db")
        sql = db.cursor()

        sql.execute("""CREATE TABLE IF NOT EXISTS pills (
    ...
    def add_new_account_query(self, name, time, repeate):
        # sql_query = "INSERT INTO accounts (Login, Password) VALUES (?, ?)"
        # self.execute_query_with_params(sql_query, [login, password])
        db = sqlite3.connect("server.db")
        sql = db.cursor()
        sql.execute(f"INSERT INTO pills VALUES (?, ?, ?, ?)", (None, name, time, repeate))
        db.commit()
    ...
    def delete_account_query(self, id):
        sql_query = "DELETE FROM pills WHERE ID=?"
    ...
    def search(self, login, pessword):
        ...
        for value in sql.execute("SELECT * FROM pills"):
